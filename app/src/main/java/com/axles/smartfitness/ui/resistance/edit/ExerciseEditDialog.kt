...
class ExerciseEditDialog(
	...
	override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
		...
		rootView = inflater.inflate(R.layout.exercise_edit_dialog, container, false)
	...
	private fun init() {
		...
		rootView.setsViewPager.adapter = ExerciseEditSetsListAdapter(programType, exercise)
	...
	private fun update() {
		...
		rootView.topBarBackBtn.setColorFilter(if (currentPageIndex == 0) Helper.color(R.color.gray) else Helper.color(R.color.colorPrimary))
		rootView.nextBtn.setColorFilter(if (currentPageIndex == rootView.setsViewPager.adapter!!.getItemCount()-1) Helper.color(R.color.gray) else Helper.color(R.color.colorPrimary))
