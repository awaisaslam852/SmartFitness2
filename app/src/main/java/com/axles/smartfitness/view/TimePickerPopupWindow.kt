...
class TimePickerPopupWindow(private val parent: View, private val ranges: List<List<Any>>, private val onPick: (values: List<Any>) -> Unit) {
		...
		private inner class PickerListItemViewHolder(view: View): RecyclerView.ViewHolder(view) {
			...
			private fun init() {
				with (itemView) {
					...
					pickerView.maxValue = range.size-1
					pickerView.displayedValues = range.map { it.toString() }.toTypedArray()
					pickerView.setFormatter(R.string.time_picker_formatter)
					pickerView.setDividerThickness(1)
					pickerView.setDividerDistance(34.dpToPx())

					pickerView.smoothScroll(true, 20)
					pickerView.isScrollerEnabled = true

					pickerView.isFadingEdgeEnabled = true
					pickerView.setTextColorResource(R.color.black)
